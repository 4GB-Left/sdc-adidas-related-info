...
looksCollection.write('id,shirt_id,pant_id,sock_id\n', 'utf8');
...
function writeMillionOfDataForPants(writer, encoding, callback) {
  ...
  function write() {
    ...
    do {
      i -= 1;
      id += 1;
      let shirt_id = faker.random.number({'min': 1, 'max': collectionSizes.looks});
      let pant_id = faker.random.number({'min': 1, 'max': collectionSizes.looks});
      let sock_id = faker.random.number({'min': 1, 'max': collectionSizes.looks});

      let data = `${id},${shirt_id},${pant_id},${sock_id}\n`;
